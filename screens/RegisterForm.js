import { StyleSheet, Text, View,Image,TextInput,SafeAreaView,Dimensions } from 'react-native'
...
//Gerekli Test Firebase auth bağlantı API anahtarı

let API_KEY = 'AIzaSyBNGmJDQilEn_9zIgDDSfIRib1FXnsECro'


const SignupForm = ({navigation}) => {

    // 4 adet giriş için useState yapısı kullanılıyor.
    ...
    //post request ile API kullanılması gereken fonksiyon
    const handleRegister = () => {
        ...
        if(email.length > 0 && password.length > 0){
            axios({
                ...
                console.warn(e)
                alert('There is an email like that in database')
                setError("")
            })
        }
    ...
    return  (
        <SafeAreaView style={styles.container}>
            ...
            <Image style={{height:80,width:Dimensions.get('window')+50/3,margin:20,resizeMode:'stretch'}}
        ...
        <TextInput style={styles.InputStyle}
            ...
            <View style={styles.loginStyle}>
                <Text style={styles.textStyle}>If you have account,login from here</Text>
                <Button title="Go to Login Page" onPress={() => handleLogin()} style={styles.buttonStyle}></Button>
            </View>
        <Button title="Register" onPress={handleRegister} style={styles.buttonStyle}></Button>
        </View>
    </SafeAreaView>
    )
